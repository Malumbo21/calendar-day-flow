...
DayFlow Pro 最多会使用三种独立凭据：

- 用于安装 `@dayflow-pro/*` 软件包的私有 npm token
- Resource Timeline 私有运行时依赖所需的 Pro runtime npm token
- 应用中用于激活 Pro 功能的已签名 DayFlow Pro 许可证密钥

前两个 npm token 只用于下载软件包。请将它们保存在本地 shell 或 CI 密钥中，
不要通过客户端环境变量暴露。已签名的 Pro 许可证密钥则是独立的客户端激活 token。
...
如果不使用 Resource Timeline，可以同时省略
`@dayflow-pro/resource-timeline` 和 `@dayflow-scheduler` 镜像源配置。
...
使用 Scheduler 作为唯一的完整 CSS 基础，并保持下面的导入顺序：
...
CSS 基础/reset，后加载的样式可能覆盖 Scheduler 的组件规则。常见表现包括视图切换
按钮变成默认样式、网格线过深、间距异常，或事件内容看起来像是消失了。
...
确认 `.npmrc` 同时包含两个镜像源，并且运行包管理器的 shell 或 CI 任务中同时存在
`DAYFLOW_PRO_NPM_TOKEN` 和 `DAYFLOW_PRO_RUNTIME_TOKEN`。
