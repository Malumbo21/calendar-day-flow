...
- Sobald `createSidebarPlugin()` zu den Plugins hinzugefügt wird, erscheint die Standard-Seitenleiste (`DefaultCalendarSidebar`) mit Kalenderfiltern, Schaltern zum Ein- und Ausblenden aller Kalender sowie Funktionen zum Erstellen, Bearbeiten, Löschen und Umsortieren von Gruppen.
