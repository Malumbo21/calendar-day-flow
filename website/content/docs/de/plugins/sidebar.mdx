...
| `groups`                     | `string[]`                                                     | `[]`                               | Gespeicherte Gruppennamen und -reihenfolge                  |
| `onGroupCreate`              | `(groupName: string) => void \| Promise<void>`                 | `undefined`                        | Callback beim Erstellen einer Gruppe                        |
| `onGroupRename`              | `(previousName, nextName, calendars) => void \| Promise<void>` | `undefined`                        | Callback beim Umbenennen einer Gruppe                       |
| `onGroupDelete`              | `(groupName, calendars) => void \| Promise<void>`              | `undefined`                        | Callback beim Löschen einer Gruppe                          |
| `onGroupReorder`             | `(groups: string[]) => void \| Promise<void>`                  | `undefined`                        | Callback beim Ändern der Gruppenreihenfolge                 |
...
Gruppen-Callbacks werden abgewartet, bevor eine Änderung übernommen wird. Wird ein Promise abgelehnt, bleiben die Gruppe oder ihre Reihenfolge unverändert und Änderungsdialoge geöffnet. Das Argument `calendars` enthält die aktuell zugewiesenen Kalender; `onGroupReorder` erhält die vollständige, sortierte Liste der Gruppennamen.
...
- **Kalendergruppen** – per Rechtsklick auf die Seitenleiste eine Gruppe erstellen; per Rechtsklick auf eine Gruppe können Sie sie umbenennen, nach Bestätigung löschen oder darin einen Kalender anlegen.
- **Gruppen umsortieren** – ziehen Sie Gruppenüberschriften, um die Reihenfolge zu ändern. Mit `groups` stellen Sie die gespeicherte Reihenfolge wieder her und mit `onGroupReorder` speichern Sie Änderungen.
