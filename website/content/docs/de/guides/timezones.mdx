...
DayFlow bringt ein `TimeZone`-Enum mit, damit Sie sowohl die anwendungsweite `timeZone` als auch die nur in Tages- und Wochenansicht wirksame `secondaryTimeZone` einstellen können, ohne rohe IANA-Zeichenketten im Kopf zu haben.

## Zeitzonenmodell

- `timeZone`: die primäre Zeitzone für Anzeige und Bearbeitung im gesamten Kalender
- `secondaryTimeZone`: eine zusätzliche Referenz-Zeitachse, die nur in der Tages- und Wochenansicht erscheint

Eine Änderung von `timeZone` projiziert alle Ansichten neu. Eine Änderung von `secondaryTimeZone` betrifft nur die zusätzliche Zeitachse in Tages- und Wochenansicht.

## Semantik der Callbacks

- `timeZone` ändert, wie Termine in der Oberfläche dargestellt und bearbeitet werden.
- Ein Wechsel von `timeZone` löst für sich genommen nie `onEventUpdate`, `onEventDrop` oder `onEventResize` aus.
- Aktualisierungs-Callbacks liefern den kanonischen Termin zurück, nachdem die Änderung angewendet wurde.
- Anders gesagt: Wird ein Termin aus Sydney in Shanghai betrachtet und dort in der Größe verändert, so wird die Änderung nach Shanghaier Wanduhrzeit interpretiert – die Nutzdaten des Callbacks werden jedoch in die kanonische Zeitdarstellung des Termins zurückgerechnet, bevor Sie sie speichern.
...
| Enum-Schlüssel           | IANA-Zeichenkette                |
| :----------------------- | :------------------------------- |
| **UTC/GMT**              |                                  |
| `TimeZone.UTC`           | `UTC`                            |
| **Nordamerika**          |                                  |
