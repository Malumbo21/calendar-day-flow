...
DayFlow stores a `subscribed: true` flag on the `CalendarType` object. When saving calendar state to a backend, preserve this field and restore it on the next load so the badge remains visible:
