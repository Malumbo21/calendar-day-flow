...
| `timeFormat`           | `'12h' \| '24h'`            | 任意 | —                                        | すべてのビュー、検索、コンポーネントにおけるグローバル時間表示形式。最高優先度を持ち、各ビューや検索設定の timeFormat を上書きします。       |
...
| `eventDetailTrigger`   | `'click' \| 'dbClick'`      | 任意 | `'dbClick'`                              | デスクトップでイベント詳細をシングルクリックまたはダブルクリックのどちらで開くかを制御します。タッチ入力では常にタップで開きます。           |
...
| `onEventDoubleClick(event, e)`                           | デフォルトの `eventDetailTrigger: 'dbClick'` モードでイベントがダブルクリックされたときに発火。`e.currentTarget` を外部ポップオーバーのアンカーに使え、`false` を返すと DayFlow の既定詳細パネル/ダイアログを抑止できます。 |
...
- `'dbClick'`（デフォルト）：シングルクリックでイベントを選択し、ダブルクリックで詳細パネル / ダイアログを開きます。
- `'click'`：シングルクリックですぐに詳細パネル / ダイアログを開きます。
- タッチ入力は変わりません。イベントをタップすると常に詳細を開きます。
