...
<FrameworkTabs>
  ...
  <Tab>
    ...
    <script lang="ts">
      ...
      import { createMonthView, type Event } from '@dayflow/core';
...
<FrameworkTabs>
  <Tab>
    ```tsx
    import { createEvent, useCalendarApp, createWeekView, DayFlowCalendar } from '@dayflow/react';
    import { Temporal } from 'temporal-polyfill';
  ...
  <Tab>
    ...
    export class AppComponent {
      ...
      calendar = {
        views: [createWeekView()],
        events: [this.sharedEvent, this.crossCalendarDay],
...
組み込みのイベント詳細パネルとダイアログは、`event.meta` を介して繰り返し設定を読み書きします。`recurring: true` を設定し、RRULE 形式の `recurrenceRule` を指定します。

| ルール項目 | 意味                                               | 例               |
| :--------- | :------------------------------------------------- | :--------------- |
| `FREQ`     | 必須の頻度：`DAILY`、`WEEKLY`、`MONTHLY`、`YEARLY` | `FREQ=WEEKLY`    |
| `INTERVAL` | N 単位ごとに繰り返す。デフォルトは `1`             | `INTERVAL=2`     |
| `BYDAY`    | カスタムの毎週ルールで選択する曜日                 | `BYDAY=MO,WE,FR` |
| `UNTIL`    | `YYYYMMDD` 形式の終了日（その日を含む）            | `UNTIL=20261231` |
| `COUNT`    | シリーズ開始を含む最大出現回数                     | `COUNT=10`       |
...
月ビューは、表示範囲内でこれらのルールを展開します。生成された繰り返しイベントは表示専用でシリーズの親イベントへの参照を保持するため、現在の編集と削除はシリーズ全体に適用されます。未対応の RRULE 項目は可能な限り組み込みエディターで保持されますが、組み込みの月ビュー展開に反映されるのは上表の項目だけです。1 回分だけを変更する例外編集にはまだ対応していません。
