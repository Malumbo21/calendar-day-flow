...
| `timeFormat`    | 検索結果表示に使用する時間形式 (`'12h'` または `'24h'`)。`useCalendarApp` にグローバルな `timeFormat` が設定されている場合は、それが優先されます。                                                                                                                                                            |
...
- `createSidebarPlugin()` を追加すると、カレンダーの絞り込み、一括表示切替、グループの作成・編集・削除・並べ替えに対応したデフォルトのサイドバーが描画されます。
