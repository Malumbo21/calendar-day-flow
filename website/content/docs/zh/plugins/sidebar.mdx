...
| `groups`                     | `string[]`                                                     | `[]`                               | 恢复已保存的分组名称和顺序          |
| `onGroupCreate`              | `(groupName: string) => void \| Promise<void>`                 | `undefined`                        | 创建分组时触发的回调                |
| `onGroupRename`              | `(previousName, nextName, calendars) => void \| Promise<void>` | `undefined`                        | 重命名分组时触发的回调              |
| `onGroupDelete`              | `(groupName, calendars) => void \| Promise<void>`              | `undefined`                        | 删除分组时触发的回调                |
| `onGroupReorder`             | `(groups: string[]) => void \| Promise<void>`                  | `undefined`                        | 分组顺序变化时触发的回调            |
...
执行分组变更前会等待对应回调完成。如果回调返回的 Promise 被拒绝，分组及其顺序将保持不变，相关弹窗也会继续显示。`calendars` 参数包含当前属于该分组的所有日历；`onGroupReorder` 会收到完整且已排序的分组名称列表。
...
- **日历分组** - 右键侧边栏可新建分组；右键分组可重命名、二次确认后删除，或直接在该分组中创建日历。
- **分组排序** - 拖动分组标题即可调整顺序。使用 `groups` 恢复已保存的顺序，并通过 `onGroupReorder` 持久化变更。
