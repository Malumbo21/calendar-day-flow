...
| `timeFormat`           | `'12h' \| '24h'`            | 可选 | —                              | 全局时间显示格式（'12h' 或 '24h'）。拥有最高优先级，设置后将覆盖视图（createXView）和搜索（search）中的不同 format 配置。 |
...
| `eventDetailTrigger`   | `'click' \| 'dbClick'`      | 可选 | `'dbClick'`                    | 控制桌面端事件详情通过单击还是双击打开。触摸端始终单击/轻触打开详情                                                       |
...
- `onEventDoubleClick(event, e)`：在默认的 `eventDetailTrigger: 'dbClick'` 模式下双击事件时触发，常用于自定义弹出层锚点；可使用 `e.currentTarget` 作为锚点，返回 `false` 可阻止 DayFlow 默认详情面板/对话框打开
...
- `'dbClick'`（默认）：单击选中事件，双击打开详情面板/对话框。
- `'click'`：单击事件后立即打开详情面板/对话框。
- 触摸端不受影响：轻触事件始终会打开详情。
