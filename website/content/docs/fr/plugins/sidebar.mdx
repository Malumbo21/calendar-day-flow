...
| `groups`                     | `string[]`                                                     | `[]`                               | Restaure les noms et l'ordre des groupes                             |
| `onGroupCreate`              | `(groupName: string) => void \| Promise<void>`                 | `undefined`                        | Callback appelé lors de la création d'un groupe                      |
| `onGroupRename`              | `(previousName, nextName, calendars) => void \| Promise<void>` | `undefined`                        | Callback appelé lors du renommage d'un groupe                        |
| `onGroupDelete`              | `(groupName, calendars) => void \| Promise<void>`              | `undefined`                        | Callback appelé lors de la suppression d'un groupe                   |
| `onGroupReorder`             | `(groups: string[]) => void \| Promise<void>`                  | `undefined`                        | Callback appelé lors du changement d'ordre des groupes               |
...
Les callbacks de groupe sont attendus avant l'application d'une modification. Si une Promise est rejetée, le groupe ou son ordre reste inchangé et les boîtes de dialogue de modification restent ouvertes. L'argument `calendars` contient les calendriers actuellement affectés ; `onGroupReorder` reçoit la liste complète et ordonnée des noms de groupes.
...
- **Groupes de calendriers** : faites un clic droit sur la barre latérale pour créer un groupe ; faites un clic droit sur un groupe pour le renommer, le supprimer après confirmation ou y créer un calendrier.
- **Réorganiser les groupes** : faites glisser les en-têtes de groupe pour modifier leur ordre. Utilisez `groups` pour restaurer l'ordre enregistré et `onGroupReorder` pour persister les changements.
