...
- Lorsque `createSidebarPlugin()` est ajouté aux plugins, la barre latérale par défaut (`DefaultCalendarSidebar`) s'affiche avec les filtres de calendrier, les contrôles « tout afficher / tout masquer » et la création, la modification, la suppression et la réorganisation des groupes.
