...
| `timeFormat`           | `'12h' \| '24h'`            | Optional | —                                                        | Global time format for all views, search, and components. Has highest priority and overrides view-level or search-level format configs. |
...
| `eventDetailTrigger`   | `'click' \| 'dbClick'`      | Optional | `'dbClick'`                                              | Controls whether desktop event details open on single click or double click. Touch input always opens details with a single tap.        |
...
- `onEventDoubleClick(event, e)` fires when an event is double-clicked in the default `eventDetailTrigger: 'dbClick'` mode. Use `e.currentTarget` as an anchor for external popovers, and return `false` to suppress DayFlow's default detail panel/dialog.
...
- `'dbClick'` (default): a single click selects the event, and a double click opens the detail panel/dialog.
- `'click'`: a single click opens the detail panel/dialog immediately.
- Touch input is unchanged: tapping an event opens details regardless of this option.
