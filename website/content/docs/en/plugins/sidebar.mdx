...
| `groups`                     | `string[]`                                                     | `[]`                               | Restores persisted group names and order        |
| `onGroupCreate`              | `(groupName: string) => void \| Promise<void>`                 | `undefined`                        | Callback invoked when creating a group          |
| `onGroupRename`              | `(previousName, nextName, calendars) => void \| Promise<void>` | `undefined`                        | Callback invoked when renaming a group          |
| `onGroupDelete`              | `(groupName, calendars) => void \| Promise<void>`              | `undefined`                        | Callback invoked when deleting a group          |
| `onGroupReorder`             | `(groups: string[]) => void \| Promise<void>`                  | `undefined`                        | Callback invoked when group order changes       |
...
Group callbacks are awaited before the group mutation is applied. If a callback rejects, the group or its order remains unchanged; mutation dialogs stay open. The `calendars` argument contains the calendars currently assigned to that group. `onGroupReorder` receives the complete ordered group-name list.
...
- **Calendar Groups** - Right-click the sidebar to create a group, or right-click a group to rename it, delete it with confirmation, or create a calendar inside it.
- **Group Reordering** - Drag group headers to reorder groups. Use `groups` to restore the saved order and `onGroupReorder` to persist changes.
