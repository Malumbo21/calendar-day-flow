...
| `timeFormat`    | Time format used for search results display (`'12h'` or `'24h'`). If a global `timeFormat` is configured on `useCalendarApp`, it takes precedence over this setting.                                                                                                                                                             |
...
- When `createSidebarPlugin()` is added to plugins, the default sidebar (`DefaultCalendarSidebar`) is rendered with calendar filters, toggle-all controls, and group creation, editing, deletion, and reordering.
