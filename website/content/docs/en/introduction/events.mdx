...
| `conference`  | `EventConference`                                                        | Optional video meeting join details. The built-in detail UI detects known providers from `joinUrl`.                                                                                                   | Optional |
...
Attach a join link with the first-class `conference` field. The built-in event
detail panel and dialog recognize Google Meet, Zoom, Microsoft Teams, Webex,
Whereby, and Jitsi links, then show the provider, meeting ID, and a safe Join
action. Other valid HTTP(S) links use a generic video-meeting presentation.
...
const event: Event = {
  ...
  conference: {
    ...
    // provider and meetingId are optional; the built-in UI can derive them.
  },
};
```

Use `createEventConference(value)` when you want the normalized URL and detected
...
The built-in event detail panel and dialog read and write recurrence settings through `event.meta`. Set `recurring: true` and provide an RRULE-style `recurrenceRule`:

| Rule part  | Meaning                                                       | Example          |
| :--------- | :------------------------------------------------------------ | :--------------- |
| `FREQ`     | Required frequency: `DAILY`, `WEEKLY`, `MONTHLY`, or `YEARLY` | `FREQ=WEEKLY`    |
| `INTERVAL` | Repeat every N frequency units; defaults to `1`               | `INTERVAL=2`     |
| `BYDAY`    | Weekdays for a custom weekly rule                             | `BYDAY=MO,WE,FR` |
| `UNTIL`    | Inclusive end date in `YYYYMMDD` format                       | `UNTIL=20261231` |
| `COUNT`    | Maximum number of occurrences, including the series start     | `COUNT=10`       |
...
The Month view expands these rules inside its visible range. Generated occurrences are render-only and retain a link to the series master, so edits and deletes currently apply to the whole series. Unsupported RRULE parts are preserved when the built-in editor can do so, but only the parts listed above affect the built-in Month view expansion. Single-occurrence exceptions are not yet supported.
