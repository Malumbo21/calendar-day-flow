...
| `groups`                     | `string[]`                                                     | `[]`                               | 저장된 그룹 이름과 순서를 복원합니다             |
| `onGroupCreate`              | `(groupName: string) => void \| Promise<void>`                 | `undefined`                        | 그룹을 만들 때 호출되는 콜백입니다               |
| `onGroupRename`              | `(previousName, nextName, calendars) => void \| Promise<void>` | `undefined`                        | 그룹 이름을 바꿀 때 호출되는 콜백입니다          |
| `onGroupDelete`              | `(groupName, calendars) => void \| Promise<void>`              | `undefined`                        | 그룹을 삭제할 때 호출되는 콜백입니다             |
| `onGroupReorder`             | `(groups: string[]) => void \| Promise<void>`                  | `undefined`                        | 그룹 순서를 바꿀 때 호출되는 콜백입니다          |
...
그룹 변경은 해당 콜백이 완료된 뒤 적용됩니다. Promise가 거부되면 그룹이나 순서는 변경되지 않고 편집 대화상자도 열린 상태로 유지됩니다. `calendars` 인수에는 현재 그룹에 속한 캘린더가 포함되며, `onGroupReorder`에는 정렬된 전체 그룹 이름 목록이 전달됩니다.
...
- **캘린더 그룹** – 사이드바를 마우스 오른쪽 버튼으로 클릭해 그룹을 만들 수 있습니다. 그룹을 마우스 오른쪽 버튼으로 클릭하면 이름 변경, 확인 후 삭제, 그룹 안에 캘린더 만들기를 할 수 있습니다.
- **그룹 순서 변경** – 그룹 헤더를 끌어 순서를 바꿉니다. `groups`로 저장된 순서를 복원하고 `onGroupReorder`로 변경 사항을 저장하세요.
